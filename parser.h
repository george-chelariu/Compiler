...
class Parser{
  ...
  private:
   ...
   int error();
   // a secoundarry error function that looks in name first then Lex and returns when it has found a Token that is in the follow set with that tokens depth
   // this error function does not report to admin the others do
   int error2nd();
   //a double loop function seeing if the current token is in the follow vectors, looking at Lex then name
   int stop();
   //a double loop function seeing if the current token is in the follow vectors, looking at name then Lex
   int stop2nd();
...
//hold the follow set when that thing is a set string of characters like "read" "write" "const" some times ";" as well
   vector <string> stopLex;
